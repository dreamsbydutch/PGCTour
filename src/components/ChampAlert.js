...
export default function ChampAlert(props) {
    const leaderboards = [props.tourney.pgcLeaderboard.filter(obj => obj.TourID === '2' && (obj.ShowRk === "1" || obj.ShowRk === "T1")),props.tourney.pgcLeaderboard.filter(obj => obj.TourID === '1' && (obj.ShowRk === "1" || obj.ShowRk === "T1"))]
    let golfers = [
                    ...
                    {leaderboards[0].map((obj,i) => (
                        <>
                            ...
                            <div className='grid grid-flow-row grid-cols-10'>
                                ...
                                <div className="text-center font-varela place-self-center py-1 px-1 col-span-4 row-span-2 font-bold text-xl text-amber-900 xs:text-2xl sm:text-3xl">{obj.Name}{littlefucker(props.data.standings.filter(a => a.TeamName === obj.Name)[0],props.data.allTourneys)}</div>
                                <div className="text-center font-varela place-self-center py-1 px-1 col-span-3 row-span-2 font-bold text-lg text-amber-900 xs:text-xl sm:text:2xl md:text-3xl">{formatScore(obj.Score)}</div>
                                {golfers[0][i].map(obj => <div className="text-center font-varela place-self-center py-1 px-1 col-span-2 row-span-1 font-bold text-2xs text-amber-900 2xs:text-xs md:text-sm">{obj.playerName}<br />({formatScore(obj.currentScore)})</div>)}
                            </div>
                        </>
                    ))}
                </div>
                    ...
                    {leaderboards[1].map((obj,i) => (
                        <>
                            ...
                            <div className='grid grid-flow-row grid-cols-10'>
                                ...
                                <div className="text-center font-varela place-self-center py-1 px-1 col-span-4 row-span-2 font-bold text-xl text-amber-900 xs:text-2xl sm:text-3xl">{obj.Name}{littlefucker(props.data.standings.filter(a => a.TeamName === obj.Name)[0],props.data.allTourneys)}</div>
                                <div className="text-center font-varela place-self-center py-1 px-1 col-span-3 row-span-2 font-bold text-lg text-amber-900 xs:text-xl sm:text:2xl md:text-3xl">{formatScore(obj.Score)}</div>
                                {golfers[1][i].map(obj => <div className="text-center font-varela place-self-center py-1 px-1 col-span-2 row-span-1 font-bold text-2xs text-amber-900 2xs:text-xs md:text-sm">{obj.playerName}<br />({formatScore(obj.currentScore)})</div>)}
                            </div>
                        </>
                    ))}
                </div>
