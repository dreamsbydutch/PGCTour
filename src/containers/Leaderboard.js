...
import { littlefucker } from './Standings'
						...
						<div className="my-4 mx-auto text-center">
							...
							<button
								...
								className={`${dbydEffect && 'animate-toggleClick'} my-2 mx-3 py-1 px-6 rounded-lg text-lg md:text-xl sm:px-8 md:px-10 font-bold ${
									leaderboardToggle === 'DbyD' ? 'bg-slate-700 text-slate-200 shadow-btn' : 'bg-slate-200 text-slate-700 shadow-btn'
								}`}
		...
		<div className="my-0 mx-1 rounded-2xl">
			<div className="grid grid-flow-col grid-cols-2 text-center mx-auto mt-4">
				<a href="#/leaderboard?tour=PGC">
					<div className="border-r border-black pr-1 bg-yellow-50 py-2">
						<div className="text-lg font-bold mb-2 text-yellow-800">Gold Playoff</div>
						...
						{pgcLeaderboardData?.map(obj => (
							<HomeLeaderboardItem info={obj} {...props} />
						))}
					</div>
				</a>
				<a href="#/leaderboard?tour=DbyD">
					<div className="pl-1 bg-slate-100 py-2">
						<div className="text-lg font-bold mb-2 text-slate-700">Silver Playoff</div>
						<div className="grid grid-flow-row grid-cols-8 text-center max-w-xl mx-auto">
							<div className="col-span-2 text-xs font-semibold font-varela place-self-center">Rank</div>
							<div className="col-span-4 text-sm font-semibold font-varela place-self-center">Name</div>
							<div className="col-span-2 text-xs font-semibold font-varela place-self-center">Score</div>
		...
		<div id="leaderboard-header">
			<div className="grid grid-flow-row grid-cols-10 border-b-2 border-gray-800 max-w-2xl mx-auto">
				<div className="text-center font-varela place-self-center py-2 px-1 col-span-3 row-span-4 max-h-40">
					<img className="max-h-36" alt="Tournament logo" src={props.tourney.Logo}></img>
				</div>
				...
				<div className="row-span-1 col-span-2 font-varela text-2xs text-center place-self-center xs:text-xs md:text-sm lg:text-base">
					{props.tourney.Location}
				</div>
				<div className="row-span-1 col-span-2 font-varela text-2xs text-center place-self-center xs:text-xs md:text-sm lg:text-base">
					{props.tourney.ShowPar}
				</div>
				...
				<div className="mt-2 row-span-1 col-span-2 font-varela text-2xs text-center place-self-center xs:text-xs md:text-sm lg:text-base">
					{props.tourney.PointsPurse} points
				</div>
				<div className="mt-2 row-span-1 col-span-2 font-varela text-2xs text-center place-self-center xs:text-xs md:text-sm lg:text-base">
					{Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(props.tourney.MoneyPurse)}
				</div>
			</div>
		</div>
	)
